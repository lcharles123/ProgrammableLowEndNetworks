...
class ExerciseRunner:
    ...
    def run_exercise(self):
        ...
        h1, h2, h3, h4, h5, h6, h7, h8, h9, h10  = self.net.hosts[0], self.net.hosts[1], self.net.hosts[2], self.net.hosts[3], self.net.hosts[4], self.net.hosts[5], self.net.hosts[6], self.net.hosts[7], self.net.hosts[8], self.net.hosts[9]
        ...
        h9.cmd('sudo ethtool -K h9-eth0 gro off gso off tso off')
        h10.cmd('sudo ethtool -K h10-eth0 gro off gso off tso off')
        ...
        s1.cmd('sudo ethtool -K s1-eth4 gro off gso off tso off')
        s1.cmd('sudo ethtool -K s1-eth5 gro off gso off tso off')
        s1.cmd('sudo ethtool -K s1-eth6 gro off gso off tso off')
        ...
        print 'ping from %s to %s' % (h1.IP(), h6.IP())
        h1.cmd('ping -c10 %s' % h6.IP())
        sleep(5)
        print 'ping from %s to %s' % (h2.IP(), h7.IP())
        h2.cmd('ping -c10 %s' % h7.IP())
        sleep(5)
        print 'ping from %s to %s' % (h3.IP(), h8.IP())
        h3.cmd('ping -c10 %s' % h8.IP())
        sleep(5)
        print 'ping from %s to %s' % (h4.IP(), h9.IP())
        h4.cmd('ping -c10 %s' % h9.IP())
        sleep(5)
        print 'ping from %s to %s' % (h5.IP(), h10.IP())
        h5.cmd('ping -c10 %s' % h10.IP())
        sleep(5)
        ...
        print '=========================================='
        print ' Scenario6 TCP - 256k - Payload MTU Bytes '
        print '=========================================='

        h1.cmd('iperf3 -s -i 30 --logfile results/scenario6/LTP-TCP-256k-1448b-h1-h6.txt &')
        h2.cmd('iperf3 -s -i 30 --logfile results/scenario6/LTP-TCP-256k-1448b-h2-h7.txt &')
