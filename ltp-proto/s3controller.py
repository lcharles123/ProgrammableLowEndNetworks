                ...
                if packet_orig[0] == LTP_DEF:
                    ...
                    if ether_type == 2048:
                        ...
                        if (ip_src, ip_dst) not in ip_cache and (ip_dst, ip_src) not in ip_cache:
                            ...
                            writeIpv4BuildRules(p4info_helper, sw=s3, dev_id=DEVSRC_ID, tag_id=LTPSRC_ID, ip_src=ip_src, ip_dst=ip_dst)
                    ...
                    if ether_type == 2048:
                        ...
                        if ip_src not in arp_table:
                            ...
                            writeARPReply(p4info_helper, sw=s3, dst_ip=ip_src, dst_mac_addr=eth_src, sw_port=value, sw_port_mac=eth_dst)
                        ...
                        if (ip_src, ip_dst) not in ip_cache and (ip_dst, ip_src) not in ip_cache:
                            ...
                            writeTagBuildRules(p4info_helper, sw=s3, dev_id=DEVSRC_ID, tag_id=LTPSRC_ID, eth_src=eth_src, ip_src=ip_src, ip_dst=ip_dst, port=EGRESS_PORT)

                            # This is the rule for the ipv4 forward table: it matches the tag, and has as action the ipv4_build, which strips the
                            # tag header and creates back the ether and ipv4 headers (using as eth_src the mac addres of the switch)
                            writeIpv4BuildRules(p4info_helper, sw=s3, dev_id=DEVSRC_ID, tag_id=LTPSRC_ID, ip_src=ip_dst, ip_dst=ip_src)
